        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categories.map((cat) => (
            <Link
              ...
              className="group bg-secondary rounded-sm p-10 text-center hover:bg-accent transition"
            >
              <h3 className="text-2xl font-medium group-hover:text-primary">
                {cat.label}
              </h3>
            </Link>
          ))}
      ...
      <section className="container mx-auto px-4 pt-20 pb-12 ">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground">
            Les bijoux les plus appréciés de notre collection
          </p>
        </div>
          ...
          {loading ? (
            Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="animate-pulse">
                <div className="bg-secondary h-48 mb-4 rounded"></div>
                <div className="bg-secondary h-4 mb-2"></div>
                <div className="bg-secondary h-4 w-2/3"></div>
              </div>
            ))
          ) : (
            bestSellers.map((product) => (
              <div key={product.id} className="group">
                <Link to={`/produit/${product.id}`}>
                  <div className="relative overflow-hidden rounded bg-secondary mb-4">
                    <img
                    ...
                    <div className="absolute top-2 right-2 bg-primary text-white px-2 py-1 text-xs rounded flex items-center gap-1">
